    ...
    if (!this.input_valid)
    {
        ...
        if (this.input.cat === NUMBER_CAT && this.number_literal_warning)
        {
                     ...
                     !IRType.box.valInRange(this.input.value, this.params))
            {
                ...
                print('box min: ' + IRType.box.getMinVal(this.params));
                print('box max: ' + IRType.box.getMaxVal(this.params));
...
// This is a temporary modification to bypass a performance problem in
// the compiler when bootstrapping.

function string_to_2darray(str)
{
    var pos = 0;
        ...
        for (;;)
        {
            ...
            if (str.charCodeAt(pos) !== 93) // "]"
            {
                for (;;)
                {
                    ...
                    while ((d = str.charCodeAt(pos)-48) >= 0 &&
                           d <= 9)
                    {
                        n = n*10 + d;
                        pos++; // skip digit
                    ...
                    if (str.charCodeAt(pos) === 44) // "," ?
                    {
                        pos++; // skip ","
                        skip_whitespace();
                    }
                    else if (str.charCodeAt(pos) === 93) // "]" ?
                    {
                        break;
                    }
                }
...
var action_table = string_to_2darray("\
[\
...
var goto_table = string_to_2darray("\
[\
