...
function initStrings()
{
    ...
    // Allocate a string buffer object
    var strbuf = alloc_strbuf(STR_BUF_DEF_SIZE);

    // Initialize the string buffer length and capacity
    set_strbuf_cap(strbuf, STR_BUF_DEF_SIZE);
    set_strbuf_len(strbuf, pint(0));

    // Get a pointer to the context
    var ctx = iir.get_ctx();

    // Set the string table and string buffer references in the context
    set_ctx_strtbl(ctx, strtbl);
    set_ctx_strbuf(ctx, strbuf);
}
...
/*
TODO: string buffer API
- strBufWrite(idx, chVal)
- strBufAppend(chVal)
- strBufCompHash() (called by strBufToStr)
  - Compute the hash code of a string buffer, for string table lookup
- strBufReset() (reset str buf state, called by strBufToStr)
  - Reset length to 0
  - Drop buffer if now bigger than initial size
- strBufToStr()
  - Auto-reset after strBufToStr()
*/

/**
Reset the state of the string buffer.
*/
function strBufReset()
{
    "tachyon:static";
    "tachyon:noglobal";
    ...
    // Get the string buffer capacity
    var cap = get_strbuf_cap(strbuf);

    // If the capacity is bigger than the default
    if (cap > STR_BUF_DEF_SIZE)
    {    
        // Allocate a string buffer object with the default size
        strbuf = alloc_strbuf(STR_BUF_DEF_SIZE);

        // Initialize the string buffer capacity
        set_strbuf_cap(strbuf, STR_BUF_DEF_SIZE);

        // Update the string buffer reference in the context
        set_ctx_strbuf(ctx, strbuf);
    }
...
/**
Write a character in the string buffer
*/
function strBufWrite(idx, chVal)
{
    ...
    if (idx >= cap)
    {
        ...
        var newCap = pint(2) * cap;
        var newbuf = alloc_strbuf(newCap);
        set_strbuf_cap(newbuf, newCap);

        // Set the string length
        set_strbuf_len(newbuf, len);

        /* 
        FIXME:
        FIXME: apparent IIR bug w.r.t phi nodes
        FIXME: write simple test for this code, test with /load
        FIXME:
        ...
        */
        ...
        // Update the string buffer reference
        strbuf = newbuf;

        // Update the string buffer reference in the context
        set_ctx_strbuf(ctx, newbuf);
    }

    // Update the string length if necessary
    if (idx >= len)
    {
        set_strbuf_len(strbuf, len);
    }

    // Write the character to the string buffer
    set_strbuf_data(strbuf, idx, chVal);
}

/**
Write append a character to the string buffer
*/
function strBufAppend(chVal)
{
    "tachyon:static";
    "tachyon:noglobal";
    "tachyon:arg chVal u16";
    ...
    // Write the character at the end of the string buffer
    strBufWrite(len, chVal);
}
...
// TODO: write new version of code using str buf, phase out old code
// only when everything is refactored
